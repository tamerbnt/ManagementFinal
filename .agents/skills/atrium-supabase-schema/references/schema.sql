...
-- Module bundle determined by p_modules (C# driven)
    ...
    INSERT INTO public.accounts (id, full_name, email, phone_number, is_active, created_at, updated_at)
    VALUES (p_owner_id, p_full_name, p_email, p_phone, true, v_now, v_now);
...
-- =============================================================================
-- PATCH v1.3: RPC 14 — get_staff_for_sync
-- =============================================================================
CREATE OR REPLACE FUNCTION public.get_staff_for_sync(
    p_account_id    uuid,
...
BEGIN
    ...
    LEFT JOIN public.staff_branch_assignments sba
        ON sba.staff_id = s.id
    WHERE s.account_id = p_account_id
      AND s.is_active  = true
      AND (p_updated_after IS NULL OR s.updated_at > p_updated_after)
      AND (p_facility_id   IS NULL OR sba.branch_id = p_facility_id OR
           (p_facility_id IS NOT NULL AND sba.branch_id IS NULL));

    RETURN COALESCE(v_result, '[]'::jsonb);
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_staff_for_sync(uuid, timestamptz, uuid)

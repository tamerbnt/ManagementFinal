...
{
    public partial class RevenueHistoryViewModel : FacilityAwareViewModelBase
    {
        ...
        [ObservableProperty]
        private RevenueHistoryDto _data = new();
        ...
        public RevenueHistoryViewModel(
            IDashboardService dashboardService,
            IReportingService reportingService,
            IModalNavigationService modalNavigationService,
            IFacilityContextService facilityContext,
            ILogger<RevenueHistoryViewModel> logger,
        ...
        private async Task LoadDataAsync()
        {
            await ExecuteLoadingAsync(async () =>
            {
                ...
                Data = await _dashboardService.GetRevenueHistoryAsync(CurrentFacilityId, start, end);
                AnalyzedPeriodLabel = Data.AnalysisPeriod;
            }, GetTerm("Terminology.Dashboard.History.LoadingRevenue") ?? "Loading lifetime revenue analytics...");
        }
        ...
        private async Task ExecuteExportPdfAsync()
        {
            await ExecuteLoadingAsync(async () =>
            {
                var pdfBytes = await _reportingService.GenerateRevenueHistoryPdfAsync(Data, CurrentFacility.ToString());
                
                // Logic to save/open PDF (simplified for this context)
                var fileName = $"Revenue_History_{CurrentFacility.ToString()}_{DateTime.Now:yyyyMMdd}.pdf";
